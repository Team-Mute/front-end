...
import { ReservationDetail } from '@/types/reservationAdmin';
...
const DetailModal: React.FC<DetailModalProps> = ({ 
  ...
  if (!reservation) {
    return (
      <InfoModal 
        isOpen={true}
        title="오류 발생"
        subtitle="예약 상세 정보를 불러오지 못했거나 정보를 찾을 수 없습니다."
        onClose={onClose}
      />
    );
  }
    ...
    const isPending = reservation.reservationStatusName === '1차 승인 대기' || reservation.reservationStatusName === '2차 승인 대기';
  ...
  const [phonePart1, phonePart2, phonePart3] = formatPhoneNumber(reservation.user.phone);
