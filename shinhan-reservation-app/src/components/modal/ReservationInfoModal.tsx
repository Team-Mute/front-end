...
interface ReservationDetails {
    ...
    reservationStatusName: '진행중' | '예약완료' | '이용완료' | '예약취소' | '반려';
    previsits: Previsit[];
...
// --- 컴포넌트 ---
export default function ReservationInfoModal({ isOpen, onClose, reservationId, status}: ReservationInfoModalProps) {
    ...
    const renderInfoContent = () => {
        ...
        if (!reservation) return <LoadingState>예약 정보를 불러올 수 없습니다.</LoadingState>;
        
        const from = formatDateTime(reservation.reservationFrom);
        const to = formatDateTime(reservation.reservationTo);
        const duration = calculateDuration(reservation.reservationFrom, reservation.reservationTo);
        const previsit = reservation.previsits.length > 0 ? formatDateTime(reservation.previsits[0].previsitFrom) : null;

        return (
            <InfoSection>
                <DetailItem>
                    <DetailLabel>이용공간</DetailLabel>
                    {reservation.spaceImageUrl && <SpaceImage src={reservation.spaceImageUrl} alt={reservation.spaceName} />}
                ...
                <InfoRow label="이용시간" value={`${from.time}~${to.time}${duration}`} />
                <InfoRow label="이용인원" value={`${reservation.reservationHeadcount}명`} />
                <InfoRow label="사용 목적" value={reservation.reservationPurpose} />
                {previsit && <InfoRow label="사전 답사 예약시간" value={`${previsit.date} ${previsit.time}`} />}
                {reservation.reservationAttachment && reservation.reservationAttachment.length > 0 && (
                    <DetailItem>
    ...
    const renderFooterButtons = () => {
        ...
        switch (reservation.reservationStatusName) {
            case '진행중':
            case '예약완료':
                return <PrimaryButton onClick={() => setIsCancelModalOpen(true)}>예약취소</PrimaryButton>;
            case '반려':
                return (
                    <>
        ...
        <>
            <ModalOverlay onClick={() => onClose()}>
                <ModalContainer onClick={(e) => e.stopPropagation()}>
                    ...
                    <ModalContent>
                        <TabContainer>
                            <TabButton isActive={activeTab === 'info'} onClick={() => setActiveTab('info')}>예약 정보</TabButton>
                            {isRejected && <TabButton isActive={activeTab === 'reason'} onClick={() => setActiveTab('reason')}>반려사유</TabButton>}
                        </TabContainer>
                        {activeTab === 'info' ? renderInfoContent() : <RejectReasonContent>{rejectMessage}</RejectReasonContent>}
                    </ModalContent>
                    <ModalFooter hasButtons={!!renderFooterButtons()}>{renderFooterButtons()}</ModalFooter>
                </ModalContainer>
            </ModalOverlay>
...
const ModalContainer = styled.div`
  width: 470px; max-height: 90vh; background: #FFFFFF;
...
const ModalTitle = styled.h1`
  font-size: 24px; font-weight: 600; color: #191F28; margin: 0;
`;
...
const DetailLabel = styled.p`
  font-size: 16px; font-weight: 500; color: #8C8F93; margin: 0;
`;
...
const FileName = styled.span`
  font-size: 16px; color: #191F28;
`;
const FileDeleteButton = styled(CloseButton)``;
const RejectReasonContent = styled.div`
  padding: 24px 20px; font-size: 16px; color: #191F28; line-height: 1.6;
`;
const LoadingState = styled.div`
  padding: 40px; text-align: center; color: #8C8F93;
`;
...
const ActionButton = styled.button`
  ...
  padding: 0px 12px; height: 46px; border-radius: 8px;
  font-weight: 500; font-size: 14px; cursor: pointer;
  background: #E8E9E9; color: #191F28; border: none;
  flex: 1;
`;
